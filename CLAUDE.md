...
```bash
./mxcli -p OwidExplorer.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p OwidExplorer.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check OwidExplorer.mpr   # Validate project
./mxcli docker check -p OwidExplorer.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p OwidExplorer.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p OwidExplorer.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p OwidExplorer.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p OwidExplorer.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p OwidExplorer.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'OwidExplorer.mpr';
```
...
./mxcli check script.mdl -p OwidExplorer.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p OwidExplorer.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p OwidExplorer.mpr

# With colored output
./mxcli lint -p OwidExplorer.mpr --color

# List available rules
./mxcli lint -p OwidExplorer.mpr --list-rules

# Output as SARIF
./mxcli lint -p OwidExplorer.mpr --format sarif > results.sarif
```
...
./mxcli report -p OwidExplorer.mpr

# JSON report
./mxcli report -p OwidExplorer.mpr --format json

# HTML report
./mxcli report -p OwidExplorer.mpr --format html
```
