-- Placeholders substituted by Owid.FetchOwidRows: ${BASE} ${ISO_CSV} ${Y0} ${Y1}
LOAD httpfs;
SET VARIABLE base = '${BASE}';
SET VARIABLE isocsv = '${ISO_CSV}';

CREATE OR REPLACE VIEW years AS SELECT unnest(generate_series(${Y0}, ${Y1})) AS year;
...
WHERE year BETWEEN ${Y0} AND ${Y1} AND gdp_per_capita IS NOT NULL;
...
-- Keep only countries that carry observations; the rest are territories with
-- no series at all and would be dead rows in every picker and ranked chart.
CREATE OR REPLACE VIEW has_data AS
SELECT country FROM observation
GROUP BY country
HAVING count(pop) > 0 AND (count(gdp) > 0 OR count(le) > 0);

-- FINAL: the Java action executes every statement above, then this one as a
-- query. Column order is load-bearing — FetchOwidRows reads by index.
SELECT o.country, r.code, o.id, o.region, o.year,
       o.pop, o.gdp, o.le, o.cm, o.en, o.co2, o.fe, o.kc, o.sc
FROM observation o
JOIN has_data h ON h.country = o.country
JOIN roster   r ON r.country = o.country
